...
export class StoaClient {
    ...
    async register(params) {
        ...
        const tx = await this.registry.register(params.capabilities, params.chainOrigin, stake);
    ...
    async post(params) {
        ...
        const approveTx = await this.usdc.approve(this.config.contracts.escrow, bounty);
        await approveTx.wait();
        // Post the job
        const tx = await this.escrow.postJob(params.task, bounty, params.requiredCapability, minRep, deadline);
        const receipt = await tx.wait();
        // Extract job ID from event
        const event = receipt.logs.find((l) => {
            try {
                return this.escrow.interface.parseLog(l)?.name === "JobPosted";
            }
            catch {
                return false;
            }
    ...
    async submitOutput(jobId, output) {
    ...
    // ── Polling ───────────────────────────────
    onJobAvailable(capability, handler, intervalMs = 5000) {
        const seen = new Set();
